...
# Tiny non-zero angle prevents OpenGL arc degeneracy when a segment should be straight
STRAIGHT_ARC_ANGLE = 1e-3
...
def get_arc_lines(
    ...
    colors = color_gradient([BLUE_B, GREEN_B], len(x_values))
        ...
        if config.renderer == RendererType.OPENGL:
            ...
            truncated_conic = ArcPolygon(
                ...
                arc_config=[
                    ...
                    {"angle": STRAIGHT_ARC_ANGLE, "color": stroke_color},
                    {"angle": -theta * DEGREES, "color": stroke_color},
                    {"angle": STRAIGHT_ARC_ANGLE, "color": stroke_color},
                ],
            )
...
class One(ThreeDScene):
    def construct(self):
        ...
        self.play(FadeOut(rect_prism), FadeIn(net), run_time=1)
...
class Two(ThreeDScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(FadeIn(axes), FadeIn(graph)),
            run_time=2,
            lag_ratio=0.4,
        )
        self.add(surface)
                ...
                net = ArcPolygon(
                    *base_points,
                    ...
                    arc_config=[
                        ...
                        {"angle": STRAIGHT_ARC_ANGLE, "color": RED},
                        {"angle": -45 * DEGREES, "color": RED},
                        {"angle": STRAIGHT_ARC_ANGLE, "color": RED},
                    ],
                )
            return net.rotate(PI / 2).next_to(axes, RIGHT, buff=1.5)

        truncated_net = always_redraw(build_truncated_net)

        self.add(truncated_net)
        self.wait(2)
        ...
        self.remove(surface, cone, truncated_cone)
...
class ThreePers1(ThreeDScene):
    def construct(self):
        ...
        graph2 = plane.plot(sa_func, x_range=[0, 4], color=BLUE)
        ...
        self.wait()
        self.remove(conic_approx)
        self.add(truncated_area)
        self.play(
